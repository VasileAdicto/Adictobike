...
// --- SHARE MENU ---
const ShareMenu = ({ buildName = 'My Dream Bike', message = "Look at this! Its my dream!", buildUrl = 'https://adictobike.vercel.app' }: { buildName?: string, message?: string, buildUrl?: string }) => {
  ...
  const channels = [
    ...
    {
      label: 'Instagram',
      action: () => { navigator.clipboard.writeText(decodeURIComponent(shareText)); window.open('https://www.instagram.com/', '_blank'); },
      icon: (
        <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute bottom-14 left-1/2 -translate-x-1/2 bg-zinc-950 border border-white/10 rounded-2xl p-4 shadow-2xl z-50 w-max"
          >
            <div className="flex gap-5 items-start">
              {channels.map((ch) => (
                <button
                  key={ch.label}
                  onClick={() => { ch.action(); if (ch.label !== 'Copy Link' && ch.label !== 'Copied!') setOpen(false); }}
                  className="flex flex-col items-center gap-2 group"
                >
                  <div className="w-11 h-11 rounded-2xl bg-zinc-900 border border-white/5 flex items-center justify-center text-zinc-500 group-hover:text-white group-hover:border-white/20 transition-all active:scale-90">
                    {ch.icon}
                  </div>
                  <span className="text-[7px] font-black uppercase tracking-widest text-zinc-600 group-hover:text-zinc-400 transition-colors whitespace-nowrap">
                    {ch.label}
                  </span>
                </button>
              ))}
            </div>
            <button onClick={() => setOpen(false)} className="absolute -top-2 -right-2 w-5 h-5 bg-zinc-800 rounded-full text-zinc-500 text-[9px] flex items-center justify-center font-black hover:bg-red-600 hover:text-white transition-colors">✕</button>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
      ...
      <motion.div initial={{ y: -50 }} animate={{ y: 0 }} className="bg-zinc-900 border-b border-white/5 p-2 flex gap-3 items-center justify-center backdrop-blur-md">
        ...
        <div className="flex gap-1">
          <label className="cursor-pointer bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded text-[9px] font-black uppercase flex items-center gap-1 italic transition-all">
            ...
            <input type="file" className="hidden" onChange={handleSingleUpload} accept={selectedCat === 'excel' ? ".xlsx,.xls" : selectedCat === 'sounds' ? "audio/*,.mp3" : "image/*"} />
          </label>
          <label className="cursor-pointer bg-zinc-800 text-zinc-300 px-2 py-1 rounded text-[9px] font-bold uppercase hover:bg-zinc-700 flex items-center gap-1 italic">
            ...
            <input type="file" className="hidden" multiple onChange={(e) => handleUpload(e, false)} accept={selectedCat === 'excel' ? ".xlsx,.xls" : selectedCat === 'sounds' ? "audio/*,.mp3" : "image/*"} />
          </label>
          <label className="cursor-pointer bg-zinc-800 text-zinc-300 px-2 py-1 rounded text-[9px] font-bold uppercase hover:bg-zinc-700 flex items-center gap-1 italic">
...
const Visualizer = ({ selectedComponents, offsets, showGrid, gridSize, isZoomed, zoomScale, steps, selectedFrameId }: any) => {
  ...
  return (
    <div id="bike-visualizer" className="relative w-full h-full bg-white rounded-none lg:rounded-[2.5rem] overflow-hidden border-0 lg:border border-zinc-200 shadow-[0_0_100px_rgba(0,0,0,0.15)] flex items-center justify-center cursor-crosshair">
      {showGrid && (
      ...
      try {
        ...
        if (compactParam) {
          ...
          ids.forEach(compId => {
            ...
            if (stepId) restored[stepId] = compId;
          });
          setSelections(restored);
          setIsFinished(true);
        }
    ...
    <div className="h-screen bg-black text-white font-sans selection:bg-red-600 overflow-hidden flex flex-col lg:overscroll-none lg:touch-none">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-black/80 backdrop-blur-2xl border-t border-white/5 z-40 shrink-0">
        <div className="max-w-[1500px] mx-auto px-4 lg:px-6 py-4 lg:py-6 grid grid-cols-12 gap-2 items-center">
          ...
          <div className="col-span-3 flex flex-col items-end justify-end gap-0.5">
            <button
              onClick={() => {
...
const GarageShareBtn = ({ buildName }: { buildName: string }) => {
  ...
  const items = [
    {
      label: 'WA',
      action: () => window.open(`https://wa.me/?text=${msg}%20${url}`, '_blank'),
    ...
    {
      label: 'TG',
      action: () => window.open(`https://t.me/share/url?url=${url}&text=${msg}`, '_blank'),
    ...
    {
      label: 'IG',
      action: () => { navigator.clipboard.writeText(decodeURIComponent(msg)); window.open('https://www.instagram.com/', '_blank'); },
    ...
    {
      label: copied ? '✓' : 'Copy',
      action: () => { navigator.clipboard.writeText(`Look at this! Its my dream! — ${buildName} | https://adictobike.vercel.app`); setCopied(true); setTimeout(() => { setCopied(false); setOpen(false); }, 1500); },
      icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>,
    },
  ];
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute bottom-9 right-0 bg-zinc-950 border border-white/10 rounded-2xl p-3 flex gap-3 shadow-2xl z-[9999]"
          >
            {items.map(it => (
              <button key={it.label} onClick={() => { it.action(); }}
                className="flex flex-col items-center gap-1.5 group">
                <div className="w-9 h-9 rounded-xl bg-zinc-900 border border-white/5 flex items-center justify-center text-zinc-500 group-hover:text-white group-hover:border-white/20 transition-all active:scale-90">
                  {it.icon}
                </div>
                <span className="text-[6px] font-black uppercase tracking-widest text-zinc-600 group-hover:text-zinc-400 transition-colors">{it.label}</span>
              </button>
            ))}
            <button onClick={() => setOpen(false)} className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-zinc-800 rounded-full text-zinc-500 text-[8px] flex items-center justify-center font-black hover:bg-red-600 hover:text-white transition-colors">✕</button>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
...
const FreewheelSound = ({ immediate = false }: { immediate?: boolean }) => {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const audio = new Audio('/parts/sounds/freesound_community-bike-freewheel-37985.mp3?v=5');
...
// --- SUMMARY VIEW ---
function SummaryView({ selections, rawSelections, onBack, onReset, setSavedBuilds, user, onOpenGarage, onOpenAuth }: any) {
        ...
        <div className="w-full lg:w-[280px] lg:min-w-[280px] shrink-0 flex flex-col border-t lg:border-t-0 lg:border-l border-white/5 bg-zinc-950/60 backdrop-blur-xl overflow-y-hidden">
          ...
          <div className="flex-1 flex flex-col justify-start lg:justify-center px-3 lg:px-5 py-1 lg:py-5 gap-1 lg:gap-2">
            <div className="grid grid-cols-2 lg:grid-cols-1 gap-1 lg:gap-2">
              ...
              <ShareMenu buildName={selections.find((c: any) => c.stepTitle === 'Frame')?.name} message="Look at this! Its my dream!" buildUrl={buildUrl} />
