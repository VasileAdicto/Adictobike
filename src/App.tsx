...
const Visualizer = ({ selectedComponents, offsets, showGrid, gridSize, isZoomed, zoomScale }: any) => {
  return (
    <div id="bike-visualizer" className="relative w-full h-full bg-zinc-950 flex items-center justify-center cursor-crosshair">
      {showGrid && (
    ...
    <div className="h-[100dvh] bg-black text-white font-sans selection:bg-red-600 overflow-hidden flex flex-col relative">
      ...
      <nav className="fixed top-0 left-0 right-0 h-[70px] px-6 flex justify-between items-center z-[100] bg-gradient-to-b from-black to-transparent pointer-events-none">
        ...
        <div className="pointer-events-auto">
          <button onClick={() => window.location.href='/admin?admin=true'} className="text-zinc-500 hover:text-white transition-colors"><Lock size={16} /></button>
        </div>
      </nav>
      ...
      <main className="relative z-10 h-full w-full flex flex-col pt-[80px] pointer-events-none">
        ...
        <div ref={stepsNavRef} className="flex overflow-x-auto no-scrollbar gap-x-6 px-6 py-4 shrink-0 pointer-events-auto">
          {steps.map((step, idx) => (
            <button key={step.id} onClick={() => setCurrentStepIndex(idx)} className={cn("transition-all text-[10px] font-black uppercase tracking-[0.25em] pb-1 border-b-2 whitespace-nowrap", idx === currentStepIndex ? "text-red-600 border-red-600 shadow-[0_4px_10px_rgba(255,0,0,0.2)]" : "text-white opacity-25 border-transparent")}>
              {step.title}
            </button>
          ))}
        </div>
      ...
      <div className="fixed bottom-8 left-0 right-0 z-[100] px-6 pointer-events-none">
        <div className="flex items-center justify-between max-w-[1500px] mx-auto pointer-events-auto">
          ...
          <div className="flex items-center gap-8 bg-zinc-900/60 backdrop-blur-3xl border border-white/10 px-8 py-3.5 rounded-full shadow-2xl">
...
function SummaryView({ selections, onReset }: any) {
  ...
  const handleExport = async () => {
    ...
    const disclaimer = "NOTICE: THE WEIGHT AND PRICE INDICATED ARE PRELIMINARY AND SUBJECT TO MINOR CHANGES BASED ON COMPONENT AVAILABILITY.";
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-8 text-center font-sans">
      <motion.img initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} src="/design/Logo.png" className="h-8 mb-10 object-contain" />
      ...
      <div className="flex flex-col gap-4 w-full max-w-[280px]">
        <button onClick={handleExport} disabled={isExporting} className="relative h-14 bg-zinc-900 border border-white/10 rounded-2xl font-black uppercase text-[10px] italic overflow-hidden shadow-xl">
          <motion.div className="absolute left-0 top-0 bottom-0 bg-red-600/80 z-0" animate={{ width: `${progress}%` }} />
          <span className="relative z-10 flex items-center justify-center gap-2 text-white">
            {isExporting ? `SAVING ${progress}%` : <><Download size={14} /> EXPORT PDF</>}
          </span>
        </button>
        <button onClick={onReset} className="text-zinc-600 hover:text-white transition-colors text-[9px] font-black uppercase tracking-[0.3em] py-2 italic">
          Build another one
        </button>
      </div>
    </div>
