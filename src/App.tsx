...
const UserDashboard = ({ builds, onEdit, onDelete, onClose, onLogout, onPDF }: any) => {
  return (
    <div className="fixed inset-0 z-[150] bg-black text-white flex flex-col font-sans overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-start mb-16">
            <div className="leading-tight">
              ...
              <h1 className="text-xs font-black uppercase tracking-widest text-red-600 mt-1">Garage</h1>
            </div>
            ...
            {builds.map((b: SavedBuild) => (
              <div key={b.id} className="bg-zinc-900/50 border border-white/5 rounded-[2rem] p-6 hover:border-white/10 transition-all group relative">
                <div className="flex justify-between items-start mb-4">
                   <span className="text-[8px] font-bold text-zinc-600 uppercase tracking-tighter italic">Bike ID: {b.id.slice(-4)}</span>
      ...
      <div className="p-10 border-t border-white/5 text-center bg-black">
        <p className="text-[8px] font-bold text-zinc-700 uppercase tracking-[0.3em] mb-2">Powered by Adicto.Bike All Right Reserved</p>
...
const AdminPanel = ({ categories, offsets, setOffsets, activeComponent, showGrid, setShowGrid, gridSize, setGridSize, isZoomed, setIsZoomed, zoomScale, setZoomScale, onLogout }: any) => {
  ...
  const saveToGithub = async (path: string, content: string, isJson = false) => {
    ...
    try {
      ...
      if (res.ok) { localStorage.setItem('adicto_github_token', token); return true; }
      return false;
  ...
  return (
    <div className="z-[100] sticky top-0 shadow-2xl font-sans text-white">
      <motion.div initial={{ y: -50 }} animate={{ y: 0 }} className="bg-zinc-900 border-b border-white/5 p-2 flex gap-3 items-center justify-center backdrop-blur-md">
        <div className="flex items-center gap-2 bg-black/40 px-2 py-1 rounded-lg border border-white/10 focus-within:border-red-600 transition-all text-white">
          <Key size={10} className={token ? "text-red-600" : "text-zinc-500"} />
          <input type={showToken ? "text" : "password"} placeholder="TOKEN" value={token} onChange={(e) => setToken(e.target.value)} className="bg-transparent text-[9px] w-20 outline-none font-mono uppercase" />
        ...
        <button onClick={() => saveToGithub("public/offsets.json", JSON.stringify(offsets), true)} className="bg-red-600 text-white px-3 py-1 rounded text-[9px] font-bold uppercase hover:bg-red-700 flex items-center gap-1 italic shadow-lg shadow-red-600/20"><Save size={10}/> Offsets</button>
...
export default function BikeConfigurator() {
  ...
  if (isDashboardOpen) return <UserDashboard builds={savedBuilds} onClose={() => setIsDashboardOpen(false)} onLogout={handleLogout} onDelete={(id: string) => {
  ...
  if (isFinished) return <SummaryView selections={selectedComponents} onReset={() => window.location.reload()} user={user} onLogin={() => setIsAuthModalOpen(true)} onDashboard={() => setIsDashboardOpen(true)} onSaveBuild={() => {
    ...
    const newBuild: SavedBuild = { id: Date.now().toString(), name: `${selectedComponents[0]?.brand || 'Bike'} Configuration`, date: new Date().toLocaleDateString(), components: selectedComponents, totalPrice: selectedComponents.reduce((acc, c) => acc + c.price, 0), totalWeight: selectedComponents.reduce((acc, c) => acc + c.weight, 0) };
    ...
    <div className="min-h-screen bg-black text-white font-sans selection:bg-red-600 pb-28 lg:pb-24 overflow-x-hidden">
      ...
      {isLoggedIn ? (
        ...
        <nav className="border-b border-white/5 px-4 lg:px-8 py-3 flex justify-between items-center bg-black/80 backdrop-blur-2xl sticky top-0 z-50">
          ...
          <div className="flex gap-4">
            {user ? (
              <button onClick={() => setIsDashboardOpen(true)} className="flex items-center gap-2 bg-white/5 px-3 py-1.5 rounded-full border border-white/5 text-white hover:bg-white/10">
          ...
          <div className="lg:col-span-3 flex flex-col bg-zinc-900/40 rounded-[2rem] border border-white/5 p-4 relative order-2 overflow-hidden shadow-2xl">
            ...
            {filteredOptions.length > 3 && (
              <div className="lg:hidden mt-2 flex items-center justify-center gap-1.5 text-zinc-600 opacity-60">
                <span className="text-[7px] font-black uppercase tracking-widest italic">Scroll for more</span>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-black/90 backdrop-blur-3xl border-t border-white/5 z-40 py-4 px-6">
        <div className="max-w-[1500px] mx-auto flex items-center justify-between text-white">
          ...
          <div className="w-20 flex justify-end">
            <button onClick={() => currentStepIndex < steps.length - 1 ? setCurrentStepIndex(currentStepIndex + 1) : setIsFinished(true)} className="bg-red-600 text-white p-2.5 rounded-xl font-black active:scale-95 transition-all shadow-lg shadow-red-600/30"><ChevronRight size={18} /></button>
          </div>
        </div>
      </div>
    </div>
...
function SummaryView({ selections, onReset, user, onSaveBuild, onLogin, onDashboard }: any) {
  ...
  const totalP = selections.reduce((acc: number, c: any) => acc + c.price, 0);
  const totalW = selections.reduce((acc: number, c: any) => acc + c.weight, 0);
    ...
    const interval = setInterval(() => {
      setProg(p => {
        if (p >= 100) { 
            ...
            const doc = new jsPDF();
            autoTable(doc, { startY: 135, head: [['SECTION', 'BRAND', 'WEIGHT', 'PRICE']], body: selections.map((c: any) => [c.stepTitle, c.brand, `${c.weight}g`, `€${c.price}`]) });
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-8 text-center font-sans overflow-y-auto">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl w-full">
        ...
        <h2 className="text-[26px] font-black italic uppercase tracking-tighter mb-4 leading-none text-white/90 text-white">your bike is <br/> <span className="text-red-600 uppercase">Ready</span></h2>
        <div className="flex justify-center gap-10 my-10 bg-zinc-900/40 p-6 rounded-[2.5rem] border border-white/5 shadow-2xl">
          <div><p className="text-zinc-600 text-[8px] uppercase font-black italic mb-1 text-zinc-500 tracking-widest uppercase">Price</p><p className="text-xl font-mono text-red-600 tracking-tighter font-black italic">€{totalP.toLocaleString()}</p></div>
          <div className="w-px bg-white/5" />
          <div><p className="text-zinc-600 text-[8px] uppercase font-black italic mb-1 text-zinc-500 tracking-widest uppercase">Weight</p><p className="text-xl font-mono text-white/80 tracking-tighter font-black italic">{totalW}g</p></div>
        </div>
