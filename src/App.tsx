    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-xl">
      <motion.div initial={{ scale: 0.98, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-zinc-900 border border-white/5 p-10 rounded-[2.5rem] max-w-sm w-full relative shadow-2xl">
        ...
        <div className="text-center mb-10 text-white">
          <h2 className="text-lg font-black uppercase italic tracking-widest mb-1">{step === 'email' ? 'Identification' : 'Verification'}</h2>
  ...
  return (
    <div className="fixed inset-0 z-[150] bg-black text-white flex flex-col font-sans overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-start mb-16">
            ...
            <div className="flex gap-4 items-center">
              {selected.length > 1 && (
                <button onClick={() => setView(view === 'compare' ? 'list' : 'compare')} className="bg-white text-black px-6 py-2 rounded-full font-black uppercase text-[9px] italic flex items-center gap-2 transition-all">
                  <Scale size={12}/> {view === 'compare' ? 'Back to List' : `Compare (${selected.length})`}
                </button>
              )}
          ...
          {view === 'list' ? (
            ...
            <div className="bg-zinc-900/30 rounded-[2.5rem] border border-white/5 overflow-x-auto p-8 custom-scroll-container">
              <table className="w-full text-left border-collapse min-w-[600px]">
                <thead>
                  <tr className="border-b border-white/5 text-[9px] uppercase text-zinc-500 font-black tracking-widest">
                    <th className="pb-6 px-4">Feature</th>
                    {buildsToCompare.map((b: any) => <th key={b.id} className="pb-6 px-4 text-red-600 italic text-sm">{b.name}</th>)}
                  </tr>
                </thead>
                <tbody className="text-[10px] font-bold uppercase">
                  <tr className="border-b border-white/5"><td className="py-5 px-4 text-zinc-600">Total Price</td>{buildsToCompare.map((b: any) => <td key={b.id} className="px-4 font-mono">€{b.totalPrice.toLocaleString()}</td>)}</tr>
                  <tr className="border-b border-white/5"><td className="py-5 px-4 text-zinc-600">Total Weight</td>{buildsToCompare.map((b: any) => <td key={b.id} className="px-4 font-mono text-zinc-400">{b.totalWeight}g</td>)}</tr>
                </tbody>
              </table>
            </div>
          )}
      ...
      <div className="p-10 border-t border-white/5 text-center bg-black">
        ...
        <p className="text-[8px] font-bold text-zinc-500 uppercase tracking-[0.1em]">Please contact us if you have any questions or bugs (баг) — <a href="mailto:hello@adicto.bike" className="text-white hover:text-red-600 transition-colors">hello@adicto.bike</a></p>
      </div>
    </div>
  );
};
          ...
          <div className="lg:col-span-3 flex flex-col bg-zinc-900/40 rounded-[2rem] border border-white/5 p-4 relative order-2 overflow-hidden shadow-2xl">
            ...
            {filteredOptions.length > 3 && (
              <div className="lg:hidden mt-2 flex items-center justify-center gap-1.5 text-zinc-600 opacity-60">
                <span className="text-[7px] font-black uppercase italic">Scroll for more</span>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-black/90 backdrop-blur-3xl border-t border-white/5 z-50 py-4 px-6">
        <div className="max-w-[1500px] mx-auto flex items-center justify-between">
          ...
          <div className="flex gap-6 items-center justify-center flex-1">
            ...
            <div className="text-center"><p className="text-[6px] text-zinc-600 uppercase font-black italic">Price</p><p className="font-mono text-xs text-red-600">€{selectedComponents.reduce((acc, c) => acc + c.price, 0).toLocaleString()}</p></div>
          </div>

          <div className="w-20 flex justify-end">
            <button onClick={() => currentStepIndex < steps.length - 1 ? setCurrentStepIndex(currentStepIndex + 1) : setIsFinished(true)} className="bg-red-600 text-white p-2.5 rounded-xl font-black transition-all active:scale-95 flex items-center justify-center shadow-lg shadow-red-600/20">
