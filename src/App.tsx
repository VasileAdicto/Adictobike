...
// ... (імпорти та типи залишаються такими ж)
  ...
  return (
    ...
    >
      <div className="aspect-square w-full rounded-xl bg-black/40 mb-3 overflow-hidden relative">
        ...
        {/* Галочка знову справа (top-2 right-2) */}
        {isSelected && (
...
export default function BikeConfigurator() {
  // ... (логіка useEffect та selections залишається без змін)
    ...
    <div className="min-h-screen bg-black text-white font-sans selection:bg-red-600 pb-24">
      ...
      <main className="max-w-[1500px] mx-auto px-6 pt-10">
        <div className="grid grid-cols-12 gap-10 h-[550px] items-stretch">
          ...
          {/* LEFT: VISUALIZER (займає 9 колонок) */}
          <div className="col-span-9 flex flex-col gap-6 order-1">
            {/* Лінія вибору кроків притиснута вліво (justify-start) */}
          ...
          {/* RIGHT: OPTIONS (займає 3 колонки) */}
      ...
      {/* FOOTER: ВАРТІСТЬ ТА ВАГА ТЕПЕР ЗЛІВА */}
      <div className="fixed bottom-0 left-0 right-0 bg-black/80 backdrop-blur-2xl border-t border-white/5 px-12 py-6 flex justify-between items-center z-40">
        <div className="flex items-center gap-10">
          <button 
            ...
            className="flex items-center gap-3 text-zinc-500 hover:text-white disabled:opacity-10 transition-all font-black uppercase text-[10px] tracking-widest pr-6 border-r border-white/10"
          >
            <ChevronLeft size={20} /> Back
          </button>

          {/* Блок Ціна/Вага переміщено сюди (вліво) */}
            ...
            <div className="flex flex-wrap justify-start items-center px-4 gap-x-4 gap-y-2">
              {steps.map((step, idx) => (
                <button
                  ...
                  onClick={() => jumpToStep(idx)}
                  className={cn(
                    "transition-all duration-300 text-[11px] font-black italic uppercase tracking-widest pb-1 border-b-2 whitespace-nowrap",
                    idx === currentStepIndex 
