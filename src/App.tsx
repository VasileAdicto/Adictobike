...
const useSounds = () => {
  ...
  const play = (file: string) => {
    if (isMobile) return; // no UI sounds on mobile
    try {
...
const FreewheelSound = ({ immediate = false }: { immediate?: boolean }) => {
  ...
  useEffect(() => {
    ...
    played.current = true;
    if (typeof window !== 'undefined' && window.innerWidth < 1024) return; // no sound on mobile
    const delay = immediate ? 0 : 400;
    const t = setTimeout(async () => {
      try {
        const audio = new Audio('/parts/sounds/freesound_community-bike-freewheel-37985.mp3?v=2');
...
function SummaryView({ selections, onBack, onReset, setSavedBuilds, user, onOpenGarage, onOpenAuth }: any) {
  ...
  const handleSaveBuild = () => {
    ...
    try { if (window.innerWidth >= 1024) { const a = new Audio('/parts/sounds/success.mp3?v=2'); a.volume = 1.0; a.play().catch(() => {}); } } catch {}
